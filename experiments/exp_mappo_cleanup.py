"""
Exp MAPPO-Cleanup -- MAPPO on the Cleanup Sequential Social Dilemma with CPT shaping.

Cross-algorithm replication: MAPPO on Cleanup (Leibo et al. 2017).
The environment and wrapper stack are IDENTICAL to exp_ppo_cleanup.py so that
the only variable between the two experiments is the algorithm.

=== WRAPPER STACK (same as PPO-Cleanup) ===
...
=== SWEEP GRID (same as PPO-Cleanup) ===
...
from env.cleanup_env import make_cleanup_env_with_w, is_collapsed_cleanup
from agents.cpt_wrapper import CPTRewardWrapper
from exp_ppo_cleanup import MetricsCallback, evaluate_cleanup

# --- Sweep grid (must match exp_ppo_cleanup.py exactly) ---------------------
...
# --- Env builder (identical to exp_ppo_cleanup._build_env) ------------------
...
def main():
    ...
    ap.add_argument("--out", default="docs/exp_cleanup/mappo_cleanup_raw_data.csv")
    args = ap.parse_args()
