...
from env.intervention_wrapper import InterventionWrapper
...
def record_learning_diagnostics(model, env, num_episodes=3):
    ...
    for _ in range(num_episodes):
        ...
        while env.agents:
            ...
            # Sum reward of PPO agents
            ep_rew += sum(rewards.values())
            
            # Record order quantities and inv
    ...
    return np.mean(total_order_qty), np.mean(total_inv), np.mean(total_lost_sales), np.mean(total_reward)

def train_intervention(phi=0.0, seed=0, total_timesteps=150000):
    np.random.seed(seed)
    
    # Base environment
    raw_env = MultiAgentNetInvMgmt()
    
    # Hide node_1 and script it
    interv_env = InterventionWrapper(raw_env)
    
    agents = interv_env.agents
    ...
    eval_interv = InterventionWrapper(eval_raw)
...
if __name__ == "__main__":
    print("\nRunning Intervention Experiment: Scripted node_1 (Base-Stock), PPO for Nodes 2-6...")
    diags = train_intervention(phi=0.0, seed=0, total_timesteps=150000)
    
    df = pd.DataFrame(diags)
    os.makedirs("docs/results", exist_ok=True)
    df.to_csv("docs/results/intervention_results.csv", index=False)
    
    print("\nIntervention study complete! Results saved to docs/results/intervention_results.csv")
