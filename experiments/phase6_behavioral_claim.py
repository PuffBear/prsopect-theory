...
def evaluate_and_extract_metrics(model, eval_env, num_episodes=5, base_seed=42):
    ...
    # Extract the S parameter the model converges to
...
def _run_single_seed(args):
    ...
    for i, agent in enumerate(agents):
        # The first `num_averse` agents are loss-averse (lambda=5)
        # Note: if scripted_nodes includes node_1, node_1 is hidden from PPO anyway,
        # but the wrapper still expects params for all raw_env.agents
        if i < num_averse:
            agent_params[agent] = {"lambda": 5.0, "alpha": 1.0, "beta": 1.0}
        else:
            agent_params[agent] = {"lambda": 1.0, "alpha": 1.0, "beta": 1.0}
            
    # Centralized rewards (w=1.0)
    cpt_env = CPTRewardWrapper(interv_env, agent_params, reward_scale=1.0, global_reward_weight=1.0)
    
    # Base-stock substrate S bounds [0, 500]
    ...
    return seed, num_averse, mean_S, mean_order, mean_inv, mean_ls, mean_prof, mean_bw

def run_phase6_behavioral_claim():
    print(f"\n=======================================================")
    print(f"Starting Phase 6: Behavioral Claim on Base-Stock Substrate")
    print(f"=======================================================")
    ...
    for num_averse in num_averse_list:
        for seed in seeds:
            args_list.append((seed, num_averse))
            
    results = []
    
    # We have 7 * 10 = 70 runs
    with mp.Pool(processes=min(mp.cpu_count(), 10)) as pool:
        for res in pool.imap_unordered(_run_single_seed, args_list):
            seed, num_averse, s, mq, mi, ls, prof, bw = res
            phi = num_averse / 6.0
            print(f"NumAverse:{num_averse:d} (phi:{phi:.2f}) Seed:{seed:2d} -> S:{s:6.1f} | Prof:{prof:6.1f} | Inv:{mi:6.1f} | BW:{bw:4.2f}")
            results.append({
                "NumAverse": num_averse,
                "Phi": phi,
                "Seed": seed,
                "Mean S": s,
                "Mean Order": mq,
    ...
    os.makedirs("docs/phase6_figures", exist_ok=True)
    df.to_csv("docs/phase6_figures/phase6_raw_data.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig("docs/phase6_figures/behavioral_phase_transition_ci.png", dpi=300)
    print("Saved docs/phase6_figures/behavioral_phase_transition_ci.png")

if __name__ == "__main__":
    mp.set_start_method('spawn')
    run_phase6_behavioral_claim()
