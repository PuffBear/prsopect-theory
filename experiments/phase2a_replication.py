...
def run_replication_configuration(lam, seed, total_timesteps=150000):
    print(f"\nRunning lambda={lam} on seed={seed}...")
    np.random.seed(seed)
...
if __name__ == "__main__":
    lambda_values = [1.0, 3.0, 5.0, 10.0]
    seeds = [0, 1, 2, 3, 4]
    ...
    for lam in lambda_values:
        for s in seeds:
            res = run_replication_configuration(lam, seed=s, total_timesteps=150000)
            all_results.append(res)
            
    df = pd.DataFrame(all_results)
    df.to_csv("docs/results/phase2a_replication.csv", index=False)
    print("\nPhase 2A Replication Study complete! Results saved to docs/results/phase2a_replication.csv")
