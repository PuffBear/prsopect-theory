"""
Analyze Exp B-Reduced: w_crit(λ) for λ ∈ {1,3,7} (α=β=0.88), and whether loss
aversion shifts the collapse isocline.

Outputs:
  - per-λ logistic (slope, p, McFadden R²), w_crit with run-level bootstrap CI (n=2000)
  - summary table; CI-overlap behavioral test
  - Figure 2  : three logistic curves + w_crit lines/CI  (expB_wcrit_by_lambda.png)
  - Figure 2b : w_crit vs λ with CI error bars            (expB_wcrit_vs_lambda.png)
  - expB_results.md
...
def main():
    ...
    for lam in LAMBDAS:
        ...
        r = res[lam]
        print(f"\n=== λ={lam} (n={r['n']}) per-w collapse fraction ===")
    ...
    if lo7 > hi1:
        ...
        detail = f"w_crit(λ=7) CI_lo={lo7:.3f} > w_crit(λ=1) CI_hi={hi1:.3f}"
    else:
    ...
    ax.set_title("Exp B-Reduced: collapse transition by loss aversion λ (α=β=0.88)")
    ax.legend(title="loss aversion")
    plt.tight_layout()
    f2 = os.path.join(OUTDIR, "expB_wcrit_by_lambda.png")
    plt.savefig(f2, dpi=150); print(f"\nSaved {f2}")

    # ---------- Figure 2b: w_crit vs λ with CI error bars ----------
    ...
    ax.set_xlabel("loss aversion λ"); ax.set_ylabel("w_crit (P=0.5)")
    ax.set_xticks(LAMBDAS)
    ax.set_title("w_crit vs λ (Exp B-Reduced)")
    plt.tight_layout()
    f2b = os.path.join(OUTDIR, "expB_wcrit_vs_lambda.png")
    plt.savefig(f2b, dpi=150); print(f"Saved {f2b}")

    # ---------- results md ----------
    md = ["# Exp B-Reduced — w_crit by loss aversion λ (α=β=0.88)\n",
