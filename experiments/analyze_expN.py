...
existing PT(lambda=1) 300k data.
...
Outputs:
  ...
  - side-by-side w_crit(null) vs w_crit(PT lambda=1 @300k) and behavioral_excess
...
def main():
    ...
    if os.path.exists(PT_CSV):
        ...
        pt_res = analyze(pt_df, "PT lambda=1 @300k (expA300k)")
    ...
    if pt_res:
        ...
        print(f"  w_crit(PT lambda=1 @300k)  : {pt_res['w_crit']:.3f} [{plo:.3f}, {phi:.3f}]")
    ...
    if pt_res:
        ax.plot(wg, curve(pt_res["model"], wg), color="tab:blue", ls="-", lw=2.2, label="PT lambda=1 @300k")
    ...
    ax.set_title("Null model vs PT(lambda=1): collapse transition")
    plt.tight_layout()
    ...
    # ---------- results md ----------
    md = [f"# Exp N - null model (v(x)=x) vs PT(lambda=1) @300k\n",
          f"Null model: {res_null['n']} runs. Logistic slope(w)={res_null['slope']:.3f}, "
          f"p={res_null['pval']:.3e}, McFadden R^2={res_null['r2']:.3f}.",
    ...
    if pt_res:
        ...
        md += ["\n## Comparison\n",
               ...
               f"- w_crit(PT lambda=1 @300k) = {pt_res['w_crit']:.3f} [{plo:.3f}, {phi:.3f}] "
               f"(slope={pt_res['slope']:.3f}, p={pt_res['pval']:.3e}, McFadden R^2={pt_res['r2']:.3f}, n={pt_res['n']})",
               f"- **behavioral_excess = {excess:.3f}** (w_crit(PT) - w_crit(null))\n",
               "Figure: [expN_vs_PT_comparison.png](expN_vs_PT_comparison.png)",
               "\nNote: both conditions use lambda=1, alpha=1, beta=1 (v(x)=x). The PT(lambda=1) dataset is "
